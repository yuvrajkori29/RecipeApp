...
export const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchSavedRecipes = async ()=>{
        try {
           const res =  await axios.get(`http://localhost:4000/recipes/savedRecipes/ids/${userID}`);
  ...
  const saveRecipe = async (recipeID) => {
    try {
      ...
      const res = await axios.put('http://localhost:4000/recipes/saveRecipeToUser', { recipeId: recipeID,
      userID: userID,});
      console.log(res.data);
